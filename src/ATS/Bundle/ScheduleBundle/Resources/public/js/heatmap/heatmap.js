...
function showHeatMap(data, ylabels, xlabels) {
    ...
    let width = Math.max(Math.min(window.innerWidth, 1500), 500) - margin.left - margin.right - 20,
    ...
    let valMax = d3.max(data, function(d) {return d.val; });
    let colorScale = d3.scale.linear()
    .domain([0, valMax/2, valMax])
    .range(["#FFFFDD", "#3E9583", "#1F2D86"])
    //.interpolate(d3.interpolateHcl);
    
    let heatmapWrapper = svg.append("g")
    ...
    let yLabels = heatmapWrapper.selectAll(".yLabel")
    .data(ylabels)
    .enter().append("text")
    ...
    let xLabels = heatmapWrapper.selectAll(".xLabel")
    .data(xlabels)
    .enter().append("text")
    .text(function(d) { return d; })
    ...
    let heatMap = heatmapWrapper.selectAll(".val")
    .data(data)
    .enter().append("rect")
